...
class TestPolicy(unittest.TestCase):
    ...
    def test_read_action_allowed(self):
        self.assertEqual(
            self.policy.evaluate("crm.search_customer", "support_agent").decision, "allow"
        )
    ...
    def test_refund_thresholds(self):
        self.assertEqual(
            self.policy.evaluate(
                "billing.issue_refund", "support_agent", {"amount": 20.0}
            ).decision,
            "allow",
        )
        self.assertEqual(
    ...
    def test_refund_without_amount_requires_approval(self):
        self.assertEqual(
            self.policy.evaluate("billing.issue_refund", "support_agent").decision, "ask"
        )

    def test_export_case_restricted_to_managers(self):
        self.assertEqual(
            self.policy.evaluate("audit.export_case", "support_agent").decision, "deny"
        )
        self.assertEqual(
            self.policy.evaluate("audit.export_case", "support_manager").decision, "allow"
        )

    def test_unknown_capability_denied_by_default(self):
        self.assertEqual(
            self.policy.evaluate("billing.delete_everything", "support_manager").decision, "deny"
        )
